...
def load_session_history(session_id: str) -> tuple[List[Dict], str | None]:
    ...
    if not os.path.exists(log_filename):
        return (
            [],
            f"Session log file '{log_filename}' does not exist. Starting new session.",
        )
    ...
    for entry in log_data.get("history", []):
        content = {"role": entry["role"], "parts": [{"text": entry["text"]}]}
...
def save_session_history(
    session_id: str, history: List[Dict], system_prompt: str, model_name: str
) -> tuple[bool, str | None]:
    """
    ...
    for content in history:
        ...
        if isinstance(content, dict) and "parts" in content and content["parts"]:
            ...
            text_part = (
                getattr(content, "parts", [{}])[0].get("text", "")
...
def list_sessions():
    ...
    session_ids = sorted(
        [
            f.replace("-log.json", "")
            for f in files
            if f.endswith("-log.json") and f != "azor-wal.json"
        ]
    )
    ...
    for sid in session_ids:
        ...
        try:
            with open(log_path, "r", encoding="utf-8") as f:
                ...
                last_msg_time_str = log_data.get("history", [{}])[-1].get(
                    "timestamp", "Brak daty"
                )
            ...
            sessions_data.append({"id": sid, "error": "BŁĄD ODCZYTU PLIKU"})
